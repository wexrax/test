...
Версия `6` добавляет данные, необходимые для закрытия ТЗ аналитики:

- `subscriptions.trial_start_at`, `trial_end_at`, `cancel_after_trial` - пробные периоды и автоотмена после триала;
- `subscriptions.last_used_at` - последняя ручная отметка «использовал»;
- `subscriptions.inactive_suggestion_hidden_at` - скрытие подсказки по неиспользуемой подписке;
- `subscription_usage_events` - журнал ручных отметок использования;
- `subscription_price_history` - история изменения цены подписки.

Автоотмена триала выполняется reconciliation-логикой вместе с архивными таймерами: подписка архивируется без создания транзакции списания и без изменения баланса кошелька.
